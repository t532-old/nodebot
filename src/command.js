class Command {
     ...
     */
    constructor({prefix, handler}) { this.prefix = prefix, this.list = {} }
    /**
     * bind a command
     * @param {object} params - {
    ...
    on({name, args, options, action}) {
        args = args.split(' ')
        args = {
            required: args.filter(i => i.match(/^<.+>$/)).map(i => i.split(/<|>/)[1]),
            optional: args.filter(i => i.match(/^\[[^\.]+\]$/)).map(i => i.split(/\[|\]/)[1]),
            group: args.filter(i => i.match(/^\[.+\.{3,3}\]$/))[0].split(/\[|\.{3,3}\]/)[1],
        }
    ...
    do(command) {
        ...
        this.list[name].action(args, options)
    }
}
